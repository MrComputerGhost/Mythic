...
public class AngelSandBlock extends Block {

	@SideOnly(Side.CLIENT)
	public Icon[] icons;

	public AngelSandBlock(int id) {

		super(id, Material.rock);
		setCreativeTab(Mythic.tabMyth);
		setUnlocalizedName(Strings.ANGELSANDBLOCK_NAME);
		setHardness(3.0F);
		setResistance(8.0F);
		setLightValue(1.0F);
    ...
    private void sparkle(World world, int par2, int par3, int par4)
    {
            ...
            for(int i = 0; i < 4; i++)
            {
                    ...
                    {
                    	world.spawnParticle("cloud", d1, d2, d3, 0.0D, 0.0D, 0.0D);
                    	world.spawnParticle("aura", d1, d2, d3, 0.0D, 0.0D, 0.0D);
